<script>
    ...
    let navbar, allSections, allIcons, 
        projectsIcon, postsIcon, aboutIcon, contactIcon

    const switchOnview = (sectionID) => {
        navbar.classList.remove('hidden')
        allIcons.forEach(icon => icon.classList.remove('onview'))
        switch (sectionID) {
            case 'hero':
                navbar.classList.add('hidden')
                break
            case 'projects':
                projectsIcon.classList.add('onview')
                break
            case 'posts':
                postsIcon.classList.add('onview')
                break
            case 'about':
                aboutIcon.classList.add('onview')
                break
            case 'contact':
                contactIcon.classList.add('onview')
                break
        }
    }
    ...
    onMount(() => {
        const io = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => !!entry.isIntersecting && switchOnview(entry.target.id) )
        }, { rootMargin: '-200px 0px -200px 0px' })

        allSections = document.querySelectorAll('section')
        allSections.forEach(section => { io.observe(section) })

        allIcons = document.querySelectorAll('nav ul li')
    })
</script>

<nav bind:this={navbar}>
    <ul>
        <li bind:this={projectsIcon}>
            <Button class="flex flex-wrap justify-center" scrollTo="#projects" btn={false}>
        ...
        <li bind:this={postsIcon}>
            <Button class="flex flex-wrap justify-center" scrollTo="#posts" btn={false}>
                <svg role="img" viewBox="0 0 24 24">
                    <title>Posts</title>
        ...
        <li bind:this={aboutIcon}>
            <Button class="flex flex-wrap justify-center" scrollTo="#about" btn={false}>
                <svg role="img" viewBox="0 0 24 24">
                    <title>About</title>
        ...
        <li bind:this={contactIcon}>
            <Button class="flex flex-wrap justify-center" scrollTo="#contact" btn={false}>
                <svg role="img" viewBox="0 0 24 24">
                    <title>Contact</title>
...
<style>
    nav {
        @apply absolute bottom-0 z-50 w-screen h-20 bg-black lg:h-14 lg:top-0;
    ...
    span {
        @apply w-full mt-1 text-sm uppercase font-fira lg:pb-1 lg:normal-case;
    }
    ...
    :global(li.onview span) {
        @apply lg:border-b-4 lg:border-prime;
    }
